...
interface InfiniteGalleryProps {
	...
	onImageClick?: (image: { src: string; alt?: string }, index: number) => void;
}
			...
			{planesData.current.map((plane, i) => {
				...
				return (
					<ImagePlane
						...
						scale={scale}
						material={material}
						onClick={() => onImageClick?.(normalizedImages[plane.imageIndex], plane.imageIndex)}
					/>
				);
			})}
		</>
...
export default function InfiniteGallery({
	...
	useEffect(() => {
		const timer = setTimeout(() => setCanvasLoaded(true), 150);
...
// Suppress THREE.Clock deprecation warnings emitted by @react-three/fiber until R3F migrates to THREE.Timer
if (typeof window !== 'undefined') {
	const origWarn = console.warn;
	console.warn = (...args: unknown[]) => {
		if (typeof args[0] === 'string' && (args[0].includes('Clock: This module has been deprecated') || args[0].includes('THREE.Clock: This module has been deprecated'))) {
			return;
		}
